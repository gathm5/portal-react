...
export default React.createClass({
	...
	componentDidMount() {
		storage.session.get('transactions').then((rows) => {
			this.setState({
				rows
			});
		}, () => {
	...
	processList({data}) {
		...
		storage.session.set("transactions", data);
	},
	...
	render() {
			...
			content = (
				<div className="recent-history container bg-white py-3 drop-shadow animated fadeIn">
					<Table
						...
						title={Lang.transactions.table.title}
		...
		return (
			<div className="transactions bg-faded pb-4">
				<Header title="Dashboard" animate/>
				<HeaderSecondary searchClicked={(input, h) => this.searchClicked(input)} animate/>
